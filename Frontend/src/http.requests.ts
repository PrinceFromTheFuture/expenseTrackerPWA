...
const apiURL = import.meta.env.VITE_API_BASE_URI as
  | string
  | undefined;
...
export const HTTPGetAllBudgets = async () => {
  const res = await axios.get<Bugdet[]>(
    `${apiURL}/budgets`
  );
  return res.data;
};
...
export const HTTPPostNewTransaction = async (
  filledForm: Omit<Transaction, "id">
) => {
...
export const HTTPGetSpendingsInTimeFrame =
  async (timeFrame: { from: string; to: string }) => {

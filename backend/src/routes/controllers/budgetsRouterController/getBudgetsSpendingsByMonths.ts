...
import { MonthsNames } from "@/types/types.js";
...
const getUserBalance = async (req: Request, res: Response) => {
  ...
  const monthsAgo = 12;
  let data: { month: MonthsNames; year: number; data: { budgetId: string; amountInAgorot: number }[] }[] = [];
...
type Data = { month: MonthsNames; year: number; data: { budgetId: string; amountInAgorot: number }[] }[];

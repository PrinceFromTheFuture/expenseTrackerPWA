...
import { getBudgetColorByIdSelector, getBudgetIconUrlByIdSelector, getBudgetNameByIdSelector } from "@/redux/budgetsSlice";
...
const ReviewBeforeSubmit = () => {
  ...
  const formDataBudgetIconUrl = useAppSelector((state) => getBudgetIconUrlByIdSelector(state, formData.budgetId!));
  const formDataBudgetColor = useAppSelector((state) => getBudgetColorByIdSelector(state, formData.budgetId!));
  const formDataPaymentMethodName = useAppSelector((state) => getPaymentMethodNameByIdSelector(state, formData.paymentMethodId!));
      ...
      <div className=" w-full ">
        ...
        <Touchable className=" mb-5 w-full bg-container rounded-lg p-4 flex justify-between items-center">
          <div className=" flex justify-start items-center gap-2 ">
            <Icon varient="full" src={`${formDataBudgetIconUrl}${formDataBudgetColor}.svg`} />
            <div>
        ...
        <div className=" mx-2">
          ...
          <div className=" flex mb-3 justify-between items-center w-full">
            ...
            <div className="text-sm text-dark text-left font-bold">{formDataPaymentMethodName}</div>
          </div>
